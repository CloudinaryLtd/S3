...
itSkipIfE2E = ENV["S3_END_TO_END"] ? true : false;
...
describe Fog do
    ...
    it "should put a streaming object to AWS (streaming v4 auth)", :skip => $itSkipIfE2E do
        streamObject = $bucket.files.create(
            :key => "awsStreamObject",
            :body => File.open(fileToStream),
    ...
    it "should get a streamed object from AWS (regular v4 auth)", :skip => $itSkipIfE2E do
        open(downloadFile, "wb") do |f|
    ...
    it "should delete a streamed object from AWS (regular v4 auth", :skip => $itSkipIfE2E do
        connection.delete_object($bucketName, "awsStreamObject")
    end

    it "should initiate a multipart upload on AWS (regular v4 auth)", :skip => $itSkipIfE2E do
        response = connection.initiate_multipart_upload(
    ...
    it "should upload a streaming part (streaming v4 auth)", :skip => $itSkipIfE2E do
        response = connection.upload_part($bucketName,
    ...
    it "should abort a multipart upload on AWS (regular v4 auth)", :skip => $itSkipIfE2E do
        connection.abort_multipart_upload(
